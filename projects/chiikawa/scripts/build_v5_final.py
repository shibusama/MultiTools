...
def apply_materials_v8():
    ...
    # 关键修正：光照方向必须是「侧掠光」
    # 实测发现 dot' 有 75% 堆在 0.95-1.0，因为光太偏向观察者，
    # 球体正面法线几乎都对着光。把光改成几乎水平的左侧掠射，
    # dot' 才会在可见面上铺开，明暗分界线才落在画面里。
    # 光方向 (1, -0.15, 0.25)：从右前方接近水平打来
    LIGHT = (1.0, -0.15, 0.25)

    # 实测该光向下：阈值 0.70 约产生 36% 阴影
    specs = {
        "Body":    ("M_Body",   col(0.98, 0.97, 0.94), 0.70, 0.78),
        "Ear_L":   ("M_Ear_L",  col(0.98, 0.97, 0.94), 0.70, 0.78),
        "Ear_R":   ("M_Ear_R",  col(0.98, 0.97, 0.94), 0.70, 0.78),
        "Eye_L":   ("M_Eye_L",  col(0.05, 0.05, 0.07), 0.50, 0.95),
        "Eye_R":   ("M_Eye_R",  col(0.05, 0.05, 0.07), 0.50, 0.95),
        "Mouth":   ("M_Mouth",  col(0.15, 0.12, 0.12), 0.55, 0.90),
        "Blush_L": ("M_Blush",  col(0.96, 0.72, 0.74), 0.70, 0.86),
        "Blush_R": ("M_Blush",  col(0.96, 0.72, 0.74), 0.70, 0.86),
    }
    for name, (mn, base, rp, sf) in specs.items():
        obj = bpy.data.objects.get(name)
        if not obj:
            continue
        m = make_toon_material_v8(mn, base, ramp_pos=rp,
                                  shadow_factor=sf, light_dir=LIGHT)
        obj.data.materials.clear()
        obj.data.materials.append(m)
        print(f"    {name} -> {mn} (阈值 {rp})")
...
def animate(arm):
    ...
    bpy.ops.object.mode_set(mode='OBJECT')
    print(f"  动画: {scene.frame_start}-{scene.frame_end}")
...
def main():
    ...
    out = os.path.join(BASE, "models", "chiikawa_v5_final.blend")
